...
from osemosys_global.utils import apply_timeshift, get_config_data
import sys
import os
...
def main(input_data: str, output_data: str, start_year: int, end_year: int, 
         seasons: Dict, dayparts: Dict, daytype: bool, timeshift: int, custom_nodes: List[str]):
    """Creates timeslice data
    ...
    try:
        ...
        demand_df = pd.read_csv(Path(INPUT_DATA_DIR, 'All_Demand_UTC_2015.csv'), encoding='latin-1')
    ...
    csp_df = pd.read_csv(Path(INPUT_DATA_DIR,
                                    'CSP 2015.csv'),
                        encoding='latin-1')
    csp_df.name = 'CSP'

    spv_df = pd.read_csv(Path(INPUT_DATA_DIR,
                                    'SolarPV 2015.csv'),
                        encoding='latin-1')
    spv_df.name = 'SPV'
    ...
    hyd_df = pd.read_csv(Path(INPUT_DATA_DIR,
    ...
    won_df = pd.read_csv(Path(INPUT_DATA_DIR,
                                    'Won 2015.csv'),
                        encoding='latin-1')
    won_df.name = 'WON'

    wof_df = pd.read_csv(Path(INPUT_DATA_DIR,
                                    'Woff 2015.csv'),
                        encoding='latin-1')
    wof_df.name = 'WOF'
    ...
    if custom_nodes:
        custom_sp_demand_profile = pd.read_csv(Path(INPUT_DATA_DIR,
                                            "custom_nodes",
    ...
    yearsplit_final = yearsplit_final.join(yearsplit.set_index('TIMESLICE'), 
                                        on = 'TIMESLICE')
    yearsplit_final.to_csv(Path(OUTPUT_DATA_DIR, 
                                        'YearSplit.csv'),
                        index=None)
    ...
    sp_demand_df_final['REGION'] = REGION
    ...
    sp_demand_df_final.to_csv(Path(OUTPUT_DATA_DIR,'SpecifiedDemandProfile.csv'), index=None)
    ...
    capfac_all_df.to_csv(Path(OUTPUT_DATA_DIR, 
                                    'CapacityFactor.csv'),
                        index=None)
    ...
    time_slice_df.to_csv(Path(OUTPUT_DATA_DIR, 
                                    'TIMESLICE.csv'),
                        index=None)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ...
        config_values = ["startYear", "endYear", "daytype", "dayparts", "seasons", "timeshift", "nodes_to_add"]
        ...
        main(
            ...
            config_data["seasons"],
            config_data["dayparts"],
            config_data["daytype"],
            int(config_data["timeshift"]),
