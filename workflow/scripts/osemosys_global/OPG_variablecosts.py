...
def main():
    ...
    # Get transmission varibale costs 

    df_varcost_trans = get_transmission_var_cost()

    # Assign name, year, and transmission variable costs to both modes 

    df_varcost_trans['REGION'] = region_name
    df_varcost_trans['MODE_OF_OPERATION'] = 1
    df_varcost_trans_copy = df_varcost_trans.copy()
    df_varcost_trans_copy['MODE_OF_OPERATION'] = 2
    df_varcost_trans = pd.concat([df_varcost_trans,df_varcost_trans_copy])
    df_varcost_trans['YEAR'] = [
        range(model_start_year, model_end_year+1)] * len(df_varcost_trans)
    df_varcost_trans = df_varcost_trans.explode('YEAR')

    # Merge variable costs dataframes 

    df_varcosts_final = df_varcost.merge(df_varcost_trans, how='outer')
    df_varcosts_final['YEAR'] = df_varcosts_final['YEAR'].astype(int)

    # ### Write out variablecost.csv

    df_varcosts_final = df_varcosts_final[['REGION', 
                           'TECHNOLOGY', 
                           'MODE_OF_OPERATION',
                           'YEAR', 
                           'VALUE']]
...
def get_transmission_var_cost():
    '''Gets electrical transmission variable cost per technology. 
    
    Returns: 
        df: DataFrame with the columns 'TECHNOLOGY' and 'VALUE' representing 
        variable cost in millions of dollars per year. 
    '''
    ...
    df = hlp.format_transmission_name(df)

    # Changes units
    # Raw given in dollars -> model units in million dollars
    df = df.rename(columns={'Annual FO&M (3.5% of CAPEX) ($2010 in $000)':'VALUE'})
