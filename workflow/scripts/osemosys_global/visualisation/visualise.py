...
def main(
    ...
    if results_by_country:
        for country in geographic_scope:
            ...
            plot_generation_annual(
                color_codes, result_data, scenario_figs_dir, country=country
            )
...
def plot_total_capacity(
    data: Dict[str, pd.DataFrame], save_dir: str, country: str = None
) -> None:
    ...
    fig = px.bar(
        ...
        labels={"YEAR": "Year", "VALUE": "Gigawatts (GW)", "LABEL": legend_title},
        title=graph_title,
    )

    fig.update_layout(
        font_family="Arial", font_size=14, legend_traceorder="reversed", title_x=0.5
    )
...
def plot_generation_annual(
    color_codes, data: Dict[str, pd.DataFrame], save_dir: str, country: str = None
) -> None:
    ...
    fig = px.bar(
        ...
        color_discrete_map=plot_colors,
        template="plotly_white",
        labels={"YEAR": "Year", "VALUE": "Petajoules (PJ)", "LABEL": legend_title},
        title=graph_title,
    )
    fig.update_layout(
        font_family="Arial", font_size=14, legend_traceorder="reversed", title_x=0.5
...
def plot_generation_hourly(
    ...
    fig = px.area(
        df,
        x="HOUR",
        y=[x for x in df.columns if x not in ["MONTH", "HOUR"]],
    ...
    fig.update_layout(
        legend_traceorder="reversed", title_x=0.5, yaxis_title="Gigawatt-Hours (GWh)"
    )
    fig["layout"]["title"]["font"] = dict(size=24)
...
def plot_transmission_capacity(
    ...
    # get node data
    df_centerpoints = df_centerpoints.rename(
        columns={"lat": "LATITUDE", "long": "LONGITUDE"}
    )
    df_centerpoints.set_index("region", inplace=True)

    if custom_nodes:
        df_custom_nodes_centerpoints = df_custom_nodes_centerpoints.rename(
            columns={"lat": "LATITUDE", "long": "LONGITUDE"}
        )
        ...
        custom_centerpoints_missing = [
            val for val in custom_nodes if val not in list(df_centerpoints.index)
        ]

        if custom_centerpoints_missing:
            print(
                f"{custom_centerpoints_missing} defined as custom nodes but not added to custom node centerpoints. Custom node transmission lines will not be plotted!"
            )

    # merge result data with node data
    trn = trn.merge(
        df_centerpoints[["LATITUDE", "LONGITUDE"]], left_on="FROM", right_index=True
    )
    trn = trn.merge(
        df_centerpoints[["LATITUDE", "LONGITUDE"]],
        left_on="TO",
        right_index=True,
        suffixes=("_FROM", "_TO"),
    )
    ...
    for y in df_year.index.unique():
        ...
        x1, y1 = (
            map_filter["LONGITUDE_FROM"].iloc[0],
            map_filter["LATITUDE_FROM"].iloc[0],
        )
        ...
        plot_map_trn_line(
            ax=ax, x1=x1, y1=y1, x2=x2, y2=y2, width=map_filter["line_width"].iloc[0]
        )

        # add capacity label
        plot_map_text(
            ax=ax, x=midco[0], y=midco[1], text=int(map_filter["VALUE"].iloc[0])
        )
...
def plot_transmission_flow(
    ...
    for _, node in enumerate(nodes_to_plot):
        plot_map_text(
            ax=ax, x=nodes_to_plot[node][1], y=nodes_to_plot[node][0], text=node
        )
...
if __name__ == "__main__":
    ...
    if "snakemake" in globals():
        ...
        custom_nodes_centerpoints = pd.read_csv(
            snakemake.input.custom_nodes_centerpoints
        )
        ...
        start_year = 2021
        end_year = 2050
        centerpoints = pd.read_csv("resources/data/default/centerpoints.csv")
        custom_nodes = []
        custom_nodes_centerpoints = pd.read_csv(
            "resources/data/custom/centerpoints.csv"
        )
        color_codes = pd.read_csv("resources/data/custom/color_codes.csv")
