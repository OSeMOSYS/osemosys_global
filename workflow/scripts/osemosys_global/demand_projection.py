...
def main(input_data: str, output_data: str, start_year: int, end_year: int, custom_nodes: List[str]):
    ...
    Import_memberships = pd.read_excel(Path(INPUT_DATA_DIR,"PLEXOS_World_2015_Gold_V1.1.xlsx") , sheet_name = "Memberships")

    # Imports SSP GDPppp and Population projections (https://tntcat.iiasa.ac.at/SspDb/dsd?Action=htmlpage&page=30)
    Import_iamc_db_GDPppp_Countries = pd.read_excel(Path(INPUT_DATA_DIR,'iamc_db_GDPppp_Countries.xlsx'))
    Import_iamc_db_POP_Countries = pd.read_excel(Path(INPUT_DATA_DIR,'iamc_db_POP_Countries.xlsx'))
    Import_iamc_db_URB_Countries = pd.read_excel(Path(INPUT_DATA_DIR,'iamc_db_URB_Countries.xlsx'))

    # Imports custom GDPppp and Population projections for countries not included in the SSP datasets.
    Import_POP_Missing = pd.read_excel(Path(INPUT_DATA_DIR,'iamc_db_POP_GDPppp_URB_Countries_Missing.xlsx') , 
                                    sheet_name = 'POP').set_index('Region')

    Import_GDP_Missing = pd.read_excel(Path(INPUT_DATA_DIR,'iamc_db_POP_GDPppp_URB_Countries_Missing.xlsx') , 
                                    sheet_name = 'GDP|PPP').set_index('Region')

    Import_URB_Missing = pd.read_excel(Path(INPUT_DATA_DIR,'iamc_db_POP_GDPppp_URB_Countries_Missing.xlsx') , 
    ...
    Import_Incl_Losses = pd.read_excel(Path(INPUT_DATA_DIR,'T&D Losses.xlsx'))
    ...
    #Checks whether PLEXOS-World 2015 data needs to be retrieved from the PLEXOS-World Harvard Dataverse.
    try:
        Open = open(Path(INPUT_DATA_DIR,'All_Demand_UTC_2015.csv'))
        
        Import_Hourly_Demand_2015 = pd.read_csv(Path(INPUT_DATA_DIR,'All_Demand_UTC_2015.csv') , encoding='latin-1')
        
    except IOError:
        urllib.request.urlretrieve ('https://dataverse.harvard.edu/api/access/datafile/3985039?format=original&gbrecs=true', 
                                    Path(INPUT_DATA_DIR,'All_Demand_UTC_2015.csv'))
    ...
    Country_Elec_OWID = pd.read_csv(Path(INPUT_DATA_DIR,'owid_pcconsumption.csv') , 
    ...
    Country_Regression = pd.merge(Country_GDPppp_WB[['Year' , 
                                                        'WB_GDPppp']] , 
                                    Country_Elec_OWID[['Year' , 
                                                    'OWID_Elec']] , 
                                    left_on = ['Year' , 
                                                'Country'] , 
                                    right_on = ['Year' , 
    ...
    if Urbanization == 'Yes':
        Country_Regression = pd.merge(Country_Regression[['Year' , 
                                                                'WB_GDPppp' , 
                                                                'OWID_Elec']] , 
                                        Country_Urb_WB[['Year' , 
                                                        'WB_Urb']] , 
    ...
    Country_Regression = pd.merge(Country_Regression, 
    ...
    for x in Country_Regression.index.unique():
        ...
        if Urbanization == 'Yes':
            ...
            sklearn_lr.fit(Country_Regression_Temp[['WB_GDPppp' , 
                                                    'WB_Urb']] , 
            ...
            Country_Regression_Temp['R2_GDPppp_Urb/Elec'] = sklearn_lr.score(Country_Regression_Temp[['WB_GDPppp' , 
                                                    'WB_Urb']] , Country_Regression_Temp['OWID_Elec'])
            
            Country_Regression_Grouped = Country_Regression_Grouped.append(Country_Regression_Temp)
        ...
        # variable (Electricity demand).    
            ...
            Country_Regression_Temp['intercept'
                                    ] ,  Country_Regression_Temp['coef_GDPppp'] = [sklearn_lr.intercept_ , 
                                                                                        float(sklearn_lr.coef_)]
            
            Country_Regression_Temp['R2_GDPppp/Elec'] = sklearn_lr.score(Country_Regression_Temp[['WB_GDPppp']] , 
                                                                    Country_Regression_Temp['OWID_Elec'])
    ...
    for a in Country_Regression_Grouped.index.unique():
        ...
        if not os.path.exists(Path(FIGS_DIR)):
            os.makedirs(FIGS_DIR)
        plt.savefig(Path(FIGS_DIR,f'{a} R2 = {w2}'+'.jpg'))
    ...
    Regression_Coefficients = Country_Regression_Grouped.loc[~Country_Regression_Grouped.index.duplicated(keep = 'first')]
    ...
    for a in Spatial_Mapping_Country['child_object'].unique():
        ...
        plt.savefig(Path(FIGS_DIR,f'Demand projection {a}'+'.jpg'))
    ...
    with open(Path(OUTPUT_DATA_DIR, 'SpecifiedAnnualDemand.csv'),'w') as f:
        ...
        for x in Node_Demand_SSP_projected_Incl_Losses.index:
            ...
            for year in range(start_year, end_year+1):
                f.write(REGION + ',' + str(FUEL) + ',' + str(year) + ',' + str(Node_Demand_SSP_projected_Incl_Losses.at[x, year]*(0.0036)) + '\n')
    ...
    if custom_nodes:
        ...
        df_demands['REGION'] = REGION
        df_demands['FUEL'] = ('ELC' +
                            df_demands['CUSTOM_NODE'] + 
                            '02')
        df_custom_demands = pd.read_csv(Path(CUSTOM_NODES_DIR,
