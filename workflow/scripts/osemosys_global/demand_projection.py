...
Import_iamc_db_GDPppp_Countries = pd.read_excel(
    os.path.join(input_data_dir, "iamc_db_GDPppp_Countries.xlsx")
)
Import_iamc_db_POP_Countries = pd.read_excel(
    os.path.join(input_data_dir, "iamc_db_POP_Countries.xlsx")
)
Import_iamc_db_URB_Countries = pd.read_excel(
    os.path.join(input_data_dir, "iamc_db_URB_Countries.xlsx")
)

# Imports custom GDPppp and Population projections for countries not included in the SSP datasets.
Import_POP_Missing = pd.read_excel(
    os.path.join(input_data_dir, "iamc_db_POP_GDPppp_URB_Countries_Missing.xlsx"),
    sheet_name="POP",
).set_index("Region")

Import_GDP_Missing = pd.read_excel(
    os.path.join(input_data_dir, "iamc_db_POP_GDPppp_URB_Countries_Missing.xlsx"),
    sheet_name="GDP|PPP",
).set_index("Region")

Import_URB_Missing = pd.read_excel(
    os.path.join(input_data_dir, "iamc_db_POP_GDPppp_URB_Countries_Missing.xlsx"),
    sheet_name="URB",
).set_index("Region")
...
# Sets spatial mapping
Spatial_Mapping = Import_memberships.copy()
Spatial_Mapping = Spatial_Mapping[Spatial_Mapping["collection"].str.contains("Region")]
Spatial_Mapping["Country"] = Spatial_Mapping.parent_object.str.split("-", expand=True)[
    1
]

Spatial_Mapping_Country = Spatial_Mapping.copy().set_index("Country")
Spatial_Mapping_Country = Spatial_Mapping_Country.loc[
    ~Spatial_Mapping_Country.index.duplicated(keep="first")
]

Spatial_Mapping_Node = Spatial_Mapping.copy().set_index("parent_object")
Spatial_Mapping_Node = Spatial_Mapping_Node.loc[
    ~Spatial_Mapping_Node.index.duplicated(keep="first")
]
...
Demand_2015_Raw = (
    Demand_2015_Raw.transpose().reset_index().rename(columns={"index": "PLEXOS_Nodes"})
)

# Adds country entry to dataframe (e.g. NA-USA-CA Node gets column entry NA-USA country)
Demand_2015_Raw.insert(
    loc=0,
    column="Country",
...
Demand_2015_Raw.insert(
    loc=1, column="PLEXOS_Countries", value=Demand_2015_Raw["PLEXOS_Nodes"].str[:6]
)

# Creates a dataframe excluding all sub-country nodes
Country_Demand_2015 = (
    Demand_2015_Raw[
        Demand_2015_Raw["PLEXOS_Countries"] == Demand_2015_Raw["PLEXOS_Nodes"]
    ]
...
Node_Demand_2015 = pd.merge(
    Demand_2015_Raw,
    Country_Demand_2015[["PLEXOS_Countries", "Country_Demand_2015"]],
    on="PLEXOS_Countries",
    how="left",
)

Node_Demand_2015["Share_%_Country_Demand"] = (
    Node_Demand_2015["Node_Demand_2015"] / Node_Demand_2015["Country_Demand_2015"]
)
...
Country_Regression = Country_Regression.dropna()

Country_Regression = pd.merge(
    Country_Regression,
    Spatial_Mapping_Country[["parent_object", "child_object"]],
    left_index=True,
    right_index=True,
)

Country_Regression = Country_Regression.set_index(Spatial_Resolution)
...
for x in Country_Regression.index.unique():
    ...
    if Urbanization == "Yes":
        ...
        (
            Country_Regression_Temp["intercept"],
            Country_Regression_Temp["coef_GDPppp"],
            Country_Regression_Temp["coef_Urb"],
        ) = [
...
for a in Country_Regression_Grouped.index.unique():
    ...
    if Urbanization == "Yes":
        ...
        w2 = Country_Regression_plot.loc[a, "R2_GDPppp_Urb/Elec"].unique()
        ...
        w = Country_Regression_plot.loc[a, "R2_GDPppp/Elec"].unique()
        plt.title(f"{a} R2 = {w}")
...
Country_POP_SSP = Import_iamc_db_POP_Countries.loc[
    (Import_iamc_db_POP_Countries["Model"] == POP_Countries_Source)
    & (Import_iamc_db_POP_Countries["Scenario"] == Pathway)
].set_index("Region")
...
Country_POP_SSP_Missing = Spatial_Mapping_Country[
    (~Spatial_Mapping_Country.index.isin(Country_POP_SSP.index))
]
...
for x in Country_POP_SSP_Missing.index:
    ...
    if a == False:
        logging.error(
            "Country data for %s is not available in the SSP or custom dataset! Add country-level data.",
            x,
        )
        raise SystemExit(1)
    else:
...
Country_POP_SSP = pd.merge(
    Spatial_Mapping_Country[["child_object"]],
    Country_POP_SSP,
    left_index=True,
    right_index=True,
    how="inner",
...
Country_GDPppp_SSP = Import_iamc_db_GDPppp_Countries.loc[
    (Import_iamc_db_GDPppp_Countries["Model"] == GDPppp_Countries_Source)
    & (Import_iamc_db_GDPppp_Countries["Scenario"] == Pathway)
].set_index("Region")
...
Country_GDPppp_SSP_Missing = Spatial_Mapping_Country[
    (~Spatial_Mapping_Country.index.isin(Country_GDPppp_SSP.index))
]
...
Country_URB_SSP = Import_iamc_db_URB_Countries.loc[
    (Import_iamc_db_URB_Countries["Model"] == URB_Countries_Source)
    & (Import_iamc_db_URB_Countries["Scenario"] == Pathway)
].set_index("Region")

# Checks whether pathway data is available for all included countries. In case pathway data is not available for all countries
# it checks wheter custom data is provided in 'iamc_db_URB_URB_Countries_Missing.xlsx'. If not, an error URBs up
# indicating for which countries data is missing. Data has to be manually added to project demand for all countries.
Country_URB_SSP_Missing = Spatial_Mapping_Country[
    (~Spatial_Mapping_Country.index.isin(Country_URB_SSP.index))
]
...
Base_df_projections = Country_GDPppp_SSP[["child_object", "Scenario"]]

Regression_Coefficients = Country_Regression_Grouped.loc[
    ~Country_Regression_Grouped.index.duplicated(keep="first")
]

if Urbanization == "Yes":
    w = ["coef_GDPppp", "coef_Urb", "intercept"]
else:
    w = ["coef_GDPppp", "intercept"]
...
Country_GDPppp_pp_SSP = pd.merge(
    Base_df_projections,
...
if Urbanization == "Yes":
    y = ["child_object", "Scenario", "coef_GDPppp", "coef_Urb", "intercept"]
else:
    y = ["child_object", "Scenario", "coef_GDPppp", "intercept"]
...
if Urbanization == "Yes":
    for z in Years_List_5:
        Country_Demand_projected_SSP[z] = (
            Country_Demand_projected_SSP["coef_GDPppp"] * Country_GDPppp_pp_SSP[z]
            + Country_Demand_projected_SSP["coef_Urb"] * Country_URB_SSP[z]
...
for a in Spatial_Mapping_Country["child_object"].unique():
    Country_Regression_Grouped_plot = Country_Regression_Grouped.loc[a]

    Country_GDPppp_pp_SSP_plot = Country_GDPppp_pp_SSP.loc[
        (Country_GDPppp_pp_SSP["child_object"] == a)
    ]

    Country_Demand_projected_SSP_plot = Country_Demand_projected_SSP.loc[
        (Country_Demand_projected_SSP["child_object"] == a)
    ]
    x = Country_Regression_Grouped_plot["WB_GDPppp"]
    ...
    m = Country_Regression_Grouped_plot["coef_GDPppp"].unique()
...
for x in Years_List_5:
    Country_Demand_projected_SSP_Aggregated[x] = (
        Country_Demand_projected_SSP[x] * Country_POP_SSP[x]
    ).round(2)

# ### Adds transmission and distribution losses to country-level demand
# Explicit modelling of domestic transmission and distribution is not incorporated in PLEXOS-World. Country-level T&D losses
# per 5-year interval are added to the projected electricity demand based on Sadovskaia et al., 2019; https://doi.org/10.1016/j.ijepes.2018.11.012. Study includes data for up till 2050. 5-year intervals after that are manually added (Maarten Brinkerink) and values kept equal compared to 2050.
...
Losses_Missing = Spatial_Mapping_Country[
    (~Spatial_Mapping_Country.index.isin(Import_Incl_Losses.Country))
]
...
Country_Demand_Incl_Losses = Country_Demand_Incl_Losses[
    Country_Demand_Incl_Losses.index.isin(Spatial_Mapping_Country.index)
...
for z in Years_List_5:
    Country_Demand_projected_SSP_Incl_Losses_Raw[z] = (
        (
            Country_Demand_projected_SSP_Aggregated[z]
            * Country_Demand_Incl_Losses[z]
            / 100
            + Country_Demand_projected_SSP_Aggregated[z]
        )
        .round(2)
...
Node_Demand_SSP_projected_Incl_Losses = pd.merge(
    Node_Demand_2015[
...
for x in Years_List_5:
    Node_Demand_SSP_projected_Incl_Losses[x] = (
        (
            Node_Demand_SSP_projected_Incl_Losses[x]
...
Node_Demand_SSP_projected_Incl_Losses = Node_Demand_SSP_projected_Incl_Losses.set_index(
    "PLEXOS_Nodes"
)

# Filters data for the to-be modelled counties.
Node_Demand_SSP_projected_Incl_Losses = Node_Demand_SSP_projected_Incl_Losses[
    (Node_Demand_SSP_projected_Incl_Losses.index.isin(Spatial_Mapping_Node.index))
]

Node_Demand_SSP_projected_Incl_Losses.insert(loc=3, column="Unit", value="GWh")
...
Node_Demand_SSP_projected_Incl_Losses_Years = Node_Demand_SSP_projected_Incl_Losses[
    Years_List
]

# Linearly interpolates values between years.
Node_Demand_SSP_projected_Incl_Losses_Years = (
    Node_Demand_SSP_projected_Incl_Losses_Years.interpolate(method="linear", axis=1)
)

# Merges dataframes.
Node_Demand_SSP_projected_Incl_Losses = pd.merge(
    Node_Demand_SSP_projected_Incl_Losses[
        ["Country", "PLEXOS_Countries", "Share_%_Country_Demand", "Unit"]
    ],
    Node_Demand_SSP_projected_Incl_Losses_Years,
    left_index=True,
    right_index=True,
...
Node_Peak_Demand_2015.columns = ["PLEXOS_Nodes", "Node_Peak_Demand_2015"]

# Calculates 2015 peak demand / total demand ratio
Node_Peak_Demand_2015["Ratio_Peak/Total_Demand_2015"] = (
    Node_Peak_Demand_2015["Node_Peak_Demand_2015"]
    / Node_Demand_2015["Node_Demand_2015"]
)
# Merges dataframes.
Node_Peak_Demand_SSP_projected = (
    pd.merge(
        Node_Peak_Demand_2015[["PLEXOS_Nodes", "Ratio_Peak/Total_Demand_2015"]],
        Node_Demand_SSP_projected_Incl_Losses,
...
for x in Years_List:
    ...
    Node_Peak_Demand_SSP_projected[x] = (
        Node_Peak_Demand_SSP_projected[x]
        * Peak_Ratio_Factor
    ...
    for x in Node_Demand_SSP_projected_Incl_Losses.index:
        ...
        for year in range(model_start_year, model_end_year + 1):
            f.write(
                ...
                + str(year)
                + ","
                + str(Node_Demand_SSP_projected_Incl_Losses.at[x, year] * (0.0036))
                + "\n"
            )
...
if custom_nodes:
    df_demands = pd.DataFrame(
        list(itertools.product(custom_nodes, years)), columns=["CUSTOM_NODE", "YEAR"]
    )
    df_demands["REGION"] = region_name
    df_demands["FUEL"] = "ELC" + df_demands["CUSTOM_NODE"] + "02"
    df_custom_demands = pd.read_csv(
        os.path.join(input_data_dir, "custom_nodes", "specified_annual_demand.csv")
    )
    df_demands = pd.merge(
        df_demands, df_custom_demands, how="left", on=["CUSTOM_NODE", "YEAR"]
    )
    df_demands = df_demands[["REGION", "FUEL", "YEAR", "VALUE"]]
    df_sp_annual_demand = pd.read_csv(
        os.path.join(output_data_dir, "SpecifiedAnnualDemand.csv")
    )
    df_demands = pd.concat([df_demands, df_sp_annual_demand], ignore_index=True)
    df_demands = apply_dtypes(df_demands, "SpecifiedAnnualDemand")
    df_demands["VALUE"] = df_demands["VALUE"].round(2)
    df_demands.drop_duplicates(
        keep="first", subset=["REGION", "FUEL", "YEAR"], inplace=True
    )
    df_demands.to_csv(
        os.path.join(output_data_dir, "SpecifiedAnnualDemand.csv"), index=None
    )
